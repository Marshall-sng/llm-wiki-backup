...
## 12. DOCX-first 目标校正：高保真是核心目标（2026-05-15）

当前正式文档导出路线需要从“能导出 DOCX”校正为“以 DOCX 高保真还原为核心目标”。

关键判断：

1. 对 DOCX、公文、制度、方案、报告等正式文稿而言，高保真不是附加优化，而是核心价值。
2. 当前 Slice C/D 已证明 DOCX 可以导出、可以保存、可以显示基础诊断，但这只是传输和保存闭环，不等于格式闭环。
3. 之前的“不要承诺高保真”只应理解为“没有证据前不要虚假声称已经高保真”，不能理解为“不做高保真”。
4. 下一阶段不应优先横向扩展 XLSX/PPTX，而应纵向打深 DOCX。

后续 DOCX-first 闭环应围绕：

- 源 DOCX 样式事实增强：字体、字号、页边距、段落缩进、行距、标题层级、编号、表格、对齐等；
- FormatSpec 规则化：把样式事实转成细粒度、可编辑、可执行的格式规则；
- Writer 落地：将规则真实写入 DOCX 样式、段落、run、numbering、table、section/page 设置；
- 反向探测：导出后重新解析 DOCX，验证哪些规则实际落地；
- 覆盖率诊断：区分已还原、部分还原、未还原、无法自动验证；
- 用户可读表达：避免长期显示 `high-fidelity-style-replica-not-supported` 这类消极边界码，改为“当前还原覆盖情况”。

约束仍然保留：

- 不恢复手动模板输入路线；
- 不自动覆盖用户文件；
- 不在无证据时声称 100% 高保真或 Word/WPS 完全一致；
- 但所有后续 DOCX 导出设计都应以提升和验证高保真为主线。

### DOCX-first staged high-fidelity execution split（2026-05-15）

为避免一次性解决“样式事实增强、writer 高保真落地、UI 诊断产品化”三个问题导致步子过大，DOCX-first 后续执行拆为：

1. E1：高保真基线诊断。先测量源 DOCX 与导出 DOCX 的样式事实、覆盖率和差距，尽量不大改 writer。
2. E2：writer 样式落地增强。基于 E1 的差距报告，优先增强主标题、标题层级、正文、编号、页边距、表格等高价值项。
3. E3：UI / 诊断产品化。将覆盖率和差距报告转成用户可读诊断，替换消极边界码。

下一步只设计和执行 E1，不一次性做完整高保真闭环。

### DOCX FormatSpec 最低基线：行文格式（通用）20210625(1).docx（2026-05-15）

`D:\llm_wiki\行文格式（通用）20210625(1).docx` 的定位不是普通参考样本，而是后续 DOCX 格式约束生成的最低标准和边界。

要求：

1. 后续生成的 DOCX FormatSpec / 格式约束，只能比该文件更细，不能比该文件更粗。
2. 对正式文稿，至少要覆盖：主标题、副标题（如存在）、一级标题、二级标题、三级标题、正文、缩进、行距、页面设置等维度。
3. `基准素材.docx` 等输出材料样本可以作为高保真目标补充更具体的样式事实，但不能削弱该通用格式基线。
4. 如果生成的格式约束缺少该基线中已有的维度，应视为格式约束生成质量问题，而不是正常降级。
5. 该基线不是恢复手动模板路线，也不是直接承诺 Word/WPS 视觉完全一致；它定义的是 FormatSpec 规则覆盖的最低标准。


## 2026-05-15 当前阶段决策：DOCX-first 高保真约束分层

当前阶段确认：DOCX 类文件的核心目标是高保真收敛。系统不得继续把“格式画像约束”作为混合 prompt 使用；必须区分证据层、规则层、写作层和导出层。基准素材与行文格式文档提供最低边界，后续 DOCX writer 应优先消费可观测的字体、字号、页边距、缩进、行距、标题层级和编号规则。无法确认的属性应进入诊断或默认基线，而不是污染目标底稿正文。
